...
// CPython `repr()` rendering for the enum-`ValueError` text (`f"{value!r} is not
// a valid TemporalMode"`) is shared with the session layer via
// `../internal/py-repr.js` (imported above) so the two layers do not keep
// drifting copies of CPython's quote-selection + escaping rules.
